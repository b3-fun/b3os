...
import {
  registerToolSafe,
  installHintInterceptor,
} from "../tools/register-tool-safe.js";
...
describe("registerToolSafe", () => {
  ...
  it("auto-prepends the call signature to the description", () => {
    ...
    expect(registered.description).toMatch(
      /^Call signature: \{ message: string, workflowId\?: string \}/,
    );
  ...
  it("accepts the canonical parameter name", async () => {
    const server = new McpServer({ name: "test", version: "1.0.0" });
    const handler = vi.fn(async (args: { message: string }) => ({
      content: [{ type: "text", text: args.message }],
    }));
    registerToolSafe(
      server,
      "test_tool",
      { description: "test", inputSchema: { message: z.string() } },
      handler,
    );
  ...
  it("coerces per-tool alias from the def.aliases field", async () => {
    const server = new McpServer({ name: "test", version: "1.0.0" });
    const handler = vi.fn(
      async (args: { definition: { nodes: Record<string, unknown> } }) => ({
    ...
    const result = await callTool(server, "test_tool", {
      workflow: { nodes: { a: 1 } },
    });
    expect(handler).toHaveBeenCalledWith(
  ...
  it("coerces snake_case to camelCase", async () => {
    ...
    registerToolSafe(
      server,
      "test_tool",
      { description: "test", inputSchema: { connectorId: z.string() } },
      handler,
    );
    installHintInterceptor(server);
...
describe("installHintInterceptor", () => {
  ...
  it("returns a hint tool result on validation failure instead of throwing", async () => {
    const server = new McpServer({ name: "test", version: "1.0.0" });
    const handler = vi.fn(async () => ({
      content: [{ type: "text", text: "reached" }],
    }));
    registerToolSafe(
      server,
      "test_tool",
  ...
  it("passes through non-validation errors unchanged", async () => {
    ...
    await expect(
      callTool(server, "test_tool", { message: "x" }),
