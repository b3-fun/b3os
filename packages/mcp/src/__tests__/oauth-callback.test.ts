...
describe("waitForOAuthCallback", () => {
  it("resolves with token and orgId when callback arrives with matching state", async () => {
    ...
    const { port, result } = await waitForOAuthCallback({ expectedState: state, timeoutMs: 5000 });

    // Simulate the browser redirecting to the callback
    await fetch(`http://127.0.0.1:${port}/callback?token=jwt-abc&orgId=org_123&state=${state}`);
...
describe("waitForOAuthCallback error cases", () => {
  it("rejects when state does not match (CSRF protection)", async () => {
    const { port, result } = await waitForOAuthCallback({ expectedState: "expected", timeoutMs: 5000 });
    await fetch(`http://127.0.0.1:${port}/callback?token=jwt&orgId=org&state=wrong`);
    await expect(result).rejects.toThrow("Invalid OAuth callback");
  });

  it("rejects when token is missing", async () => {
    const { port, result } = await waitForOAuthCallback({ expectedState: "s", timeoutMs: 5000 });
  ...
  it("times out when no callback arrives", async () => {
    const { result } = await waitForOAuthCallback({ expectedState: "s", timeoutMs: 100 });
    await expect(result).rejects.toThrow("Timed out");
  });

  it("returns 404 for unknown paths", async () => {
    const { port, result } = await waitForOAuthCallback({ expectedState: "s", timeoutMs: 2000 });
    const res = await fetch(`http://127.0.0.1:${port}/unknown`);
    expect(res.status).toBe(404);
    // Then resolve the main promise via a valid callback so the test doesn't hang
    await fetch(`http://127.0.0.1:${port}/callback?token=jwt&orgId=org&state=s`);
